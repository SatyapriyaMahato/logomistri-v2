  ...
  return (
    <div className="animate-fade-in min-h-screen bg-black pb-20">
      <div className="py-20 text-center bg-black relative overflow-hidden">
        ...
        <h1 className="relative z-10 text-5xl md:text-6xl font-bold text-white mb-2">PORTFOLIO</h1>
        <p className="relative z-10 text-gray-400 italic font-light">Professional Logo Design & Branding Projects</p>
      </div>

      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((item) => (
          <Link href={`/portfolio/${item.id}`} key={item.id} className="group cursor-pointer relative overflow-hidden aspect-square block">
            <img 
              src={item.thumbnail} 
              alt={item.title} 
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center text-white">
              ...
              <p className="text-yellow-500 tracking-widest text-sm uppercase">{item.category}</p>
            </div>
          </Link>
        ))}
      </div>
